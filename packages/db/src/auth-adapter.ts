import { PartitionKey, SqlQuerySpec } from "@azure/cosmos"
import {
  Adapter,
  AdapterSession,
  AdapterUser,
...
import { convertCosmosDocument } from "./utils"
  ...
  return {
    ...
    async getUser(id) {
      ...
      const { resource } = await users
        .item(id, options.containerOptions?.usersOptions?.body?.partitionKey as PartitionKey)
        .read()
    ...
    async getUserByAccount({ providerAccountId, provider }) {
      ...
      if (account_result && account_result.length > 0) {
        ...
        const { resource } = await (await db.users())
          .item(
            userId,
            options.containerOptions?.usersOptions?.body?.partitionKey as PartitionKey
          )
          .read()
        return convertCosmosDocument(resource)
      }
          ...
          hit_account.map(
            async (account) =>
              await accounts
                .item(
                  account.id,
                  options.containerOptions?.accountsOptions?.body?.partitionKey as PartitionKey
                )
                .delete()
          )
        )
          ...
          hit_session.map(
            async (session) =>
              await sessions
                .item(
                  session.id,
                  options.containerOptions?.sessionsOptions?.body?.partitionKey  as PartitionKey
                )
                .delete()
          )
        )
    ...
    async useVerificationToken(verificationToken) {
      ...
      if (resources.length > 0) {
        const item = tokens.item(
          resources[0].id,
          options.containerOptions?.tokensOptions?.body?.partitionKey as PartitionKey
        )
